...
export default function App() {
  ...
  // --- STATE PEMBARUAN: UNDO/REDO & SCOREBOARD ---
  ...
  const handleSaveFile = () => {
    ...
    link.href = url; 
    link.download = `Takraw_${new Date().toISOString().slice(0,10)}.json`; 
    link.click(); 
  ...
  const handleOpenFile = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        setTournamentType(data.tournamentType || 'Groups'); 
        setRoundRobinType(data.roundRobinType || 'Single Round Robin');
        ...
        setCourts(data.courts || ['LAPANGAN 1', 'LAPANGAN 2']);
  ...
  const handleCopyWhatsApp = () => {
    ...
    try { 
      ...
      alert("✅ Klasemen disalin! Silakan Paste di grup WhatsApp."); 
    } catch (err) { 
  ...
  // --- LOGIKA PEMBARUAN: SNAPSHOT UNTUK UNDO/REDO ---
  ...
  const handleUndo = () => {
    ...
    setKnockoutData(prev.knockoutData);
    setTeams(prev.teams); 
    setGroupAssignments(prev.groupAssignments); 
    setTournamentType(prev.tournamentType);
    setNumGroups(prev.numGroups); 
    setPhase1Standings(prev.phase1Standings);
  };
  ...
  const handleRedo = () => {
    ...
    setMatchHistory(next.matchHistory); 
    setKnockoutData(next.knockoutData);
    setTeams(next.teams); 
    setGroupAssignments(next.groupAssignments); 
    setTournamentType(next.tournamentType);
    setNumGroups(next.numGroups); 
    setPhase1Standings(next.phase1Standings);
  };
  ...
  const handleRemoveSponsor = (index) => { 
      const newSponsors = [...sponsorLogos]; 
      newSponsors.splice(index, 1); 
      setSponsorLogos(newSponsors); 
  };
  ...
  const handleUpdateTitle = (index, value) => { 
      const newTitles = [...championshipTitles]; 
      newTitles[index] = value; 
      setChampionshipTitles(newTitles); 
  };

  const handleAddCourt = (e) => { 
      if (e) e.preventDefault(); 
      const newCourt = courtInputValue.trim(); 
      if (!newCourt || courts.includes(newCourt)) return; 
      setCourts([...courts, newCourt]); 
      setCourtInputValue(''); 
  };

  const handleRemoveCourt = (courtToRemove) => { 
      if (courts.length === 1) return alert("Minimal 1 lapangan aktif!"); 
  ...
  const handleAddTeam = (e) => { 
      ...
      if (!newTeam || teams.includes(newTeam)) return; 
  ...
  const handleRemoveTeam = (teamToRemove) => { 
      ...
      delete newAssignments[teamToRemove]; 
      setGroupAssignments(newAssignments); 
      const newLogos = { ...teamLogos }; 
      delete newLogos[teamToRemove]; 
  ...
  // --- LOGIKA PEMBARUAN: DRAG & DROP GRUP ---
  ...
  // --- LOGIKA PEMBARUAN: REORDER JADWAL (DRAG & DROP) ---
  const reorderMatchSchedule = (srcIdx, destIdx) => {
    ...
    // Pastikan ID, Time, Date, dan Court tetap statis berurutan secara visual
    const origMeta = schedule.map(m => ({ 
        id: m.id, time: m.time, date: m.date, court: m.court 
    }));
  ...
  const getStandings = (specificSchedule = schedule, specificAssignments = groupAssignments) => {
    ...
    teams.forEach(t => { 
        standings[t] = { 
            team: t, 
            group: tournamentType === 'Groups' ? (specificAssignments[t] ? `Grup ${specificAssignments[t]}` : 'Unknown') : 'Pool Utama', 
            play: 0, win: 0, lose: 0, partyWin: 0, partyLose: 0, setWin: 0, setLose: 0, pointWin: 0, pointLose: 0, totalPoints: 0 
        }; 
    });
    ...
    filteredMatches.forEach(match => {
      ...
      if (match.winner === tA) { 
          standings[tA].win += 1; standings[tB].lose += 1; 
      } else if (match.winner === tB) { 
          standings[tB].win += 1; standings[tA].lose += 1; 
      }
      
      match.parties.forEach(party => {
        if (party.winner === tA) { 
            standings[tA].partyWin += 1; standings[tB].partyLose += 1; 
        } else if (party.winner === tB) { 
            standings[tB].partyWin += 1; standings[tA].partyLose += 1; 
        }
        
        party.sets.forEach(set => {
          const a = parseInt(set.scoreA); 
          const b = parseInt(set.scoreB);
          if (!isNaN(a) && !isNaN(b)) {
            if (a > b) { 
                standings[tA].setWin += 1; standings[tB].setLose += 1; 
            } else if (b > a) { 
                standings[tB].setWin += 1; standings[tA].setLose += 1; 
            }
            standings[tA].pointWin += a; 
            standings[tA].pointLose += b; 
            standings[tB].pointWin += b; 
            standings[tB].pointLose += a;
          }
        });
      });
      ...
      if (isTeamEvent) {
        if (match.winner === tA) { 
            standings[tA].totalPoints += (match.winsB === 1) ? 2 : 3; 
            standings[tB].totalPoints += (match.winsB === 1) ? 1 : 0; 
        } else if (match.winner === tB) { 
            standings[tB].totalPoints += (match.winsA === 1) ? 2 : 3; 
            standings[tA].totalPoints += (match.winsA === 1) ? 1 : 0; 
        }
      } else { 
  ...
  const generateSchedule = () => {
    if (tournamentType === 'Groups' && !teams.every(team => groupAssignments[team])) {
        return alert("Data grup belum lengkap!");
    }
    saveSnapshot(); 
    ...
    const createRR = (gTeams, label) => {
      ...
      let sch = []; 
      let cur = [...gTeams]; 
      if(cur.length % 2 !== 0) cur.push(null);
      const n = cur.length; 
      for(let r=0; r<n-1; r++){ 
          for(let i=0; i<n/2; i++){ 
              if(cur[i] && cur[n-1-i]) {
                  sch.push({ teamA: cur[i], teamB: cur[n-1-i], groupLabel: label, roundLabel: 'P1' }); 
              }
          } 
    ...
    if (tournamentType === 'Groups') {
      ...
      gl.forEach(g => { 
          ...
          gM[g] = m; 
          if(m.length > maxM) maxM = m.length; 
      });
      for (let i = 0; i < maxM; i++) { 
          gl.forEach(g => { 
              if (gM[g][i]) allMatches.push(gM[g][i]); 
          }); 
      }
    } else { 
    ...
    if (roundRobinType === 'Double Round Robin') {
        allMatches = [...allMatches, ...allMatches.map(m => ({ teamA: m.teamB, teamB: m.teamA, groupLabel: m.groupLabel, roundLabel: 'P2' }))];
    }
  ...
  const generateDirectKnockout = (manualTeams = teams, startRoundIdx = 0, initialId = 1) => {
    ...
    for(let r = 0; r < numRounds; r++) {
       ...
       for(let m = 0; m < matchesInRound; m++) {
          ...
          if (r > 0) {
              let pA = rounds[r-1][m*2]; 
              let pB = rounds[r-1][m*2+1];
              if (pA.winner) tA = pA.winner; 
              if (pB.winner) tB = pB.winner;
          }
          ...
          for(let p=0; p<nP; p++) {
              pts.push({ id: `k_p${p}`, label: isTeamEvent?pL[p]:`Match`, sets:[{scoreA:'',scoreB:''},{scoreA:'',scoreB:''},{scoreA:'',scoreB:''}], winner:null });
          }
          
          roundMatches.push({ 
              id: mCount++, roundIndex: r, matchIndex: m, 
              title: r===numRounds-1?'BABAK FINAL':r===numRounds-2?'SEMI FINAL':r===numRounds-3?'PEREMPAT FINAL':'PENYISIHAN', 
              teamA: tA, teamB: tB, parties: pts, winner: aW, winsA: 0, winsB: 0, isBye: isB, 
              time: 'TBD', date: defaultDate, court: 'TBD', 
              nextMatchRef: r<numRounds-1 ? { r: r+1, m: Math.floor(m/2), slot: m%2===0?'teamA':'teamB' } : null 
          });
       }
  ...
  const handleDateTimeChange = (mId, isK, rI, mI, field, val) => {
    if (isK) { 
        ...
        setSchedule(prev => prev.map(m => m.id === mId ? { ...m, [field]: val } : m)); 
    }
  };
  ...
  const handleScoreChange = (mId, pI, sI, side, val) => {
    ...
    setSchedule(prev => prev.map(m => {
      if (m.id === mId) {
        ...
        let wA=0, wB=0, z=0;
        p.sets.forEach(s => { 
            const a=parseInt(s.scoreA); 
            const b=parseInt(s.scoreB); 
            if(!isNaN(a)&&!isNaN(b)){ 
                if(a>b)wA++; else if(b>a)wB++; else if(a===0&&b===0)z++; 
            }
        });
        ...
        pts.forEach(px => { 
            if(px.winner===m.teamA) mW++; else if(px.winner===m.teamB) mL++; else if(px.winner==='SERI') mS++; 
        });
  ...
  // --- LOGIKA PEMBARUAN: SCOREBOARD LIVE & AUTO-LOCK ---
  const handleUpdatePapanSkor = (isA, amount) => {
    if (!activeScoreboard) return;
    const cur = activeScoreboard.isKnockout ? knockoutData[activeScoreboard.rI]?.[activeScoreboard.mI] : schedule.find(x => x.id === activeScoreboard.id);
    if (!cur) return;
    
    const st = cur.parties[activeScoreboard.pIndex].sets[activeScoreboard.setIndex];
    const sA = parseInt(st.scoreA) || 0; 
    const sB = parseInt(st.scoreB) || 0;
    
    // Auto Lock Penguncian Skor
    if (amount > 0 && ((sA >= 15 || sB >= 15) && Math.abs(sA - sB) >= 2 || sA === 17 || sB === 17)) {
        return alert("Set Selesai!");
    }
    
    saveSnapshot(); // Menyimpan ke Undo System
    const newVal = isA ? Math.max(0, sA + amount) : Math.max(0, sB + amount);
  ...
  const handleResetSet = () => {
    if (!activeScoreboard || !window.confirm("Reset Set?")) return;
    saveSnapshot();
    if (activeScoreboard.isKnockout) { 
  ...
  const generateMasterPlan = () => {
    ...
    if (allMatches.length > 0) {
        allMatches.forEach(m => masterPlan.push({ ...m, label: m.groupLabel, phase: "Penyisihan" }));
    } else {
    ...
    if (stage < 3) {
      ...
      if ((tournamentType === 'Groups' && Number(numGroups) === 2) || isSP) {
        ...
        knockoutData.forEach(r => r.forEach(m => masterPlan.push({ ...m, label: m.title, phase: "Knockout" }))); 
    }
  ...
  const formatMatchScore = (m) => {
    if (m.isBye) return "BYE / W.O"; 
    if (!m.winner || m.winner === '?') return "TBD";
  ...
  const renderAestheticBracket = () => {
    const isSP = tournamentType === 'Group'; 
    const isReal = knockoutData.length > 0;
  ...
  const renderTeamCard = (team) => (
    <div 
        ...
        className="bg-white p-3 pr-4 rounded-2xl flex items-center justify-between border border-gray-200 shadow-sm hover:border-blue-400 cursor-grab active:cursor-grabbing transition-colors group"
    >
       <div className="flex items-center gap-4">
          <label className={`cursor-pointer w-12 h-12 rounded-2xl bg-gray-50 border-2 border-gray-100 flex items-center justify-center overflow-hidden hover:border-gray-300 group relative`}>
              {teamLogos[team] ? <img src={teamLogos[team]} className="w-full h-full object-cover" alt="Logo" /> : <IconImage className="w-5 h-5 text-gray-300 group-hover:text-blue-500 transition-colors" />}
       ...
       <div className="flex items-center gap-3">
          {tournamentType === 'Groups' && (
             <select value={groupAssignments[team] || ''} onChange={(e) => setGroupAssignments({...groupAssignments, [team]: e.target.value})} className="bg-gray-50 border border-gray-200 text-gray-700 font-bold text-xs rounded-xl px-3 py-2 outline-none cursor-pointer focus:bg-white focus:border-gray-300 transition-colors hidden sm:block">
  ...
  const renderMatchCard = (m, isK = false, index = null) => {
    ...
    return (
      <div 
          key={m.id} 
          draggable={!isProjectorMode && !isK && !isL} 
          onDragStart={(e) => { e.dataTransfer.setData('text/plain', index); }} 
          onDragOver={(e) => e.preventDefault()} 
          onDrop={(e) => { 
              e.preventDefault(); 
              const src = e.dataTransfer.getData('text/plain'); 
              if(src !== '') reorderMatchSchedule(parseInt(src), index); 
          }} 
          className={`bg-white rounded-3xl shadow-sm border ${isL && !isProjectorMode ? 'border-red-400 shadow-md ring-4 ring-red-50' : theme.border} overflow-hidden flex flex-col transition-all duration-300 hover:shadow-md print-break-inside-avoid print:border-gray-300 print:shadow-none print:ring-0 ${isProjectorMode ? (isL ? 'border-4 border-red-500 shadow-2xl' : 'border-none shadow-xl') : ''} ${!isProjectorMode && !isK && !isL ? 'cursor-grab active:cursor-grabbing hover:ring-2 hover:ring-blue-200' : ''}`}
      >
        <div className={`p-4 text-center border-b ${isL ? 'border-red-200' : theme.border} ${m.winner ? (m.winner === 'SERI' ? 'bg-gray-100 border-transparent print:bg-gray-100' : `${theme.accent} ${theme.accentText} border-transparent print:bg-gray-100`) : isL ? 'bg-red-50/80 print:bg-white' : `bg-gray-50/50 print:bg-white`} ${isProjectorMode ? 'py-5' : ''} relative`}>
          ...
          {!isProjectorMode && !isK && index !== null && (
            <div className="no-print absolute right-0 top-1/2 -translate-y-1/2 flex flex-col gap-0.5 opacity-0 hover:opacity-100 group-hover:opacity-100 transition-opacity pr-1">
        ...
        <div className="flex justify-between items-center p-5 bg-white relative pointer-events-none">
          ...
          <div className="w-2/5 flex flex-col items-center text-center">
              <div className="w-14 h-14 bg-gray-50 rounded-2xl flex items-center justify-center mb-2 overflow-hidden">
                  {teamLogos[m.teamA] ? <img src={teamLogos[m.teamA]} className="w-full h-full object-cover" /> : <IconUsers />}
              </div>
              <span className="font-black text-sm uppercase truncate w-full">{m.teamA}</span>
          </div>
          <div className="w-2/5 flex flex-col items-center text-center">
              <div className="w-14 h-14 bg-gray-50 rounded-2xl flex items-center justify-center mb-2 overflow-hidden">
                  {teamLogos[m.teamB] ? <img src={teamLogos[m.teamB]} className="w-full h-full object-cover" /> : <IconUsers />}
              </div>
              <span className="font-black text-sm uppercase truncate w-full">{m.teamB}</span>
          </div>
        </div>
        <div className="bg-gray-50/50 p-5 flex flex-col gap-3 print:bg-white">
          {m.isBye ? <div className="p-4 text-center font-black text-emerald-500 border-2 border-emerald-100 border-dashed rounded-2xl text-sm">AUTO ADVANCE (BYE)</div> : (!isTBD && m.parties.map((p, pi) => (
            <div key={pi} className="bg-white border border-emerald-100 rounded-2xl p-3 shadow-sm print-break-inside-avoid">
  ...
  return (
    <div className={`min-h-screen uppercase ${theme.bgApp} font-sans text-gray-800 pb-20 transition-all duration-500`}>
      ...
      <main className="max-w-7xl mx-auto mt-6 px-4 space-y-6">
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 relative overflow-hidden text-center">
           ...
           <div className="flex justify-center flex-wrap gap-4 mb-8">
              {sponsorLogos.map((l, i) => ( 
                  <div key={i} className="relative group w-24 h-24 bg-white border border-gray-100 rounded-3xl p-2 shadow-sm">
                      <img src={l} className="w-full h-full object-contain" />
                      <button onClick={()=>handleRemoveSponsor(i)} className="no-print absolute -top-2 -right-2 bg-red-500 text-white p-1.5 rounded-full opacity-0 group-hover:opacity-100">
                          <IconTrash />
                      </button>
                  </div> 
              ))}
              <label className="no-print w-24 h-24 border-2 border-dashed border-gray-200 rounded-3xl flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 transition-all text-gray-400 font-bold text-[10px] uppercase">
                  <IconPlus /> Sponsor<input type="file" hidden onChange={handleAddSponsor} />
              </label>
           </div>
           <div className="space-y-2">
              ...
              <div className="flex flex-col items-center gap-2 mt-4 no-print">
                 <input value={championshipTitles[2]} onChange={e=>handleUpdateTitle(2,e.target.value)} placeholder="Lokasi & Tanggal" className="text-center font-black text-xs text-gray-400 uppercase outline-none" />
        ...
        {stage === 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 no-print">
             <div className="lg:col-span-7 bg-white rounded-3xl shadow-sm border border-gray-100 p-8 flex flex-col h-[550px] overflow-hidden">
                ...
                <form onSubmit={handleAddTeam} className="flex gap-3 mb-6 shrink-0">
                    <input value={inputValue} onChange={e=>setInputValue(e.target.value)} placeholder="Nama Tim..." className="flex-1 px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl font-black text-sm uppercase outline-none focus:border-blue-400" />
                    <button className={`${theme.primary} text-white px-6 rounded-2xl shadow-lg`}><IconPlus /></button>
                </form>
                
                {tournamentType === 'Groups' ? (
                   <div className="flex flex-col gap-4 h-full overflow-y-auto pr-2 pb-4">
                      <div className="text-[10px] font-black text-gray-400 text-center border-b border-dashed pb-2 uppercase tracking-widest">Tarik (Drag) tim ke dalam Grup</div>
                      ...
                      <div onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleTeamDropGroup(e, '')} className="bg-gray-50/80 p-4 rounded-2xl border-2 border-dashed border-gray-200 min-h-[120px]">
                         <h4 className="text-[10px] font-black text-gray-400 mb-3 text-center">BELUM MASUK GRUP</h4>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
                         {Array.from({length: Number(numGroups) || 2}, (_, i) => String.fromCharCode(65 + i)).map(g => (
                            <div key={g} onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleTeamDropGroup(e, g)} className="bg-blue-50/40 p-4 rounded-2xl border-2 border-dashed border-blue-200 min-h-[150px]">
                               <h4 className="text-[10px] font-black text-blue-700 mb-3 text-center">GRUP {g}</h4>
             ...
             <div className="lg:col-span-5 bg-white rounded-3xl shadow-sm border border-gray-100 p-8 flex flex-col gap-6">
                <div>
                    <h3 className="text-[10px] font-black text-gray-400 uppercase mb-3">Kategori Event</h3>
                    <select value={selectedEventFormat} onChange={e=>setSelectedEventFormat(e.target.value)} className="w-full bg-gray-50 p-4 rounded-2xl font-black text-sm uppercase outline-none focus:border-blue-400">
                        <option value="Regu Event">Regu Event</option>
                        <option value="Regu Event Team">Regu Event Team</option>
                        <option value="Mixed Event Team">Mixed Event Team</option>
                    </select>
                </div>
                <div>
                    <h3 className="text-[10px] font-black text-gray-400 uppercase mb-3">Sistem</h3>
                    <select value={tournamentType} onChange={e=>setTournamentType(e.target.value)} className="w-full bg-gray-50 p-4 rounded-2xl font-black text-sm uppercase outline-none focus:border-blue-400 mb-4">
                        <option value="Group">1 Pool Utama</option>
                        <option value="Groups">Banyak Grup</option>
                        <option value="Knocked Out Round">Sistem Gugur</option>
                    </select>
                    {tournamentType==='Groups' && (
                        <div className="flex items-center justify-between bg-emerald-50 p-4 rounded-2xl border border-emerald-100">
                            <span className="text-xs font-black text-emerald-800">Jumlah Grup</span>
                            <div className="flex gap-2">
                                <input type="number" min="2" max="26" value={numGroups} onChange={handleNumGroupsChange} className="w-12 text-center font-black p-2 rounded-lg outline-none" />
                                <button onClick={handleAutoAssign} className="bg-white text-[10px] font-black px-3 py-2 rounded-lg text-emerald-700 shadow-sm border border-emerald-200">ACAK</button>
                            </div>
                        </div>
                    )}
                </div>
                <button onClick={tournamentType==='Knocked Out Round'?()=>{saveSnapshot(); setKnockoutData(generateDirectKnockout());setStage(3);}:generateSchedule} disabled={teams.length<2} className={`mt-auto w-full ${theme.accent} ${theme.accentText} py-5 rounded-2xl font-black text-lg uppercase shadow-lg transition-all active:scale-95 disabled:opacity-50`}>Mulai Turnamen</button>
             </div>
          </div>
        ) : (
          <div className="space-y-6">
             <div className="no-print bg-white p-4 rounded-3xl border border-gray-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
                <div className="flex items-center gap-3">
                    <div className="p-3 bg-emerald-50 text-emerald-600 rounded-2xl"><IconTable /></div>
                    <div><h3 className="font-black text-sm">Dashboard</h3><p className="text-[10px] text-gray-400 font-bold">Input Skor & Pantau Laporan</p></div>
                </div>
                <div className="flex gap-2">
               ...
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                 {Object.entries(getStandings()).map(([gn, gt]) => (
                   <div key={gn} className="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm">
                      <div className="bg-gray-50/80 p-4 border-b border-gray-100 font-black text-center text-xs text-gray-400 uppercase tracking-widest">{gn}</div>
                      <table className="w-full text-left">
                         <thead><tr className="text-[9px] font-black text-gray-400 border-b border-gray-50 uppercase tracking-widest"><th className="p-4">#</th><th className="p-4">Tim</th><th className="p-4 text-center">P</th><th className="p-4 text-center">W</th><th className="p-4 text-center">L</th><th className="p-4 text-center">PTS</th></tr></thead>
                         <tbody>
                            {gt.map((s, i) => (
             ...
             {stage === 1 && isActivePhaseFinished && (
               <div className="no-print bg-gray-900 p-12 rounded-[40px] text-center text-white shadow-2xl">
                  ...
                  <button onClick={()=>{
                    ...
                    setStage(3);
                  }} className="bg-emerald-600 px-10 py-5 rounded-2xl font-black uppercase text-lg shadow-xl hover:bg-emerald-700 transition-all">Lanjut Fase / Sistem Gugur</button>
               </div>
             )}
          </div>
        )}
      ...
      {showMasterModal && (
        <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 master-modal-overlay animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-5xl max-h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden master-modal-content">
              ...
              <div id="master-print-area" className="flex-1 overflow-y-auto p-12 bg-white text-gray-800">
                 ...
                 <div className="mb-12">
                     <h3 className="font-black text-lg border-l-4 border-emerald-500 pl-4 mb-8">Klasemen Akhir</h3>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                         {Object.entries(getStandings()).map(([gn,gt])=>(
                             <div key={gn} className="bg-gray-50 p-6 rounded-3xl">
                                 <div className="text-center font-black text-xs text-gray-400 mb-4">{gn}</div>
                                 <table className="w-full text-xs font-bold">
                                     <tbody>
                                         {gt.map((s,i)=>(
                                             <tr key={i} className="border-b border-gray-100">
                                                 <td className="p-2 text-gray-300">#{i+1}</td>
                                                 <td className="p-2 uppercase">{s.team}</td>
                                                 <td className="p-2 text-right">{s.totalPoints} PTS</td>
                                             </tr>
                                         ))}
                                     </tbody>
                                 </table>
                 ...
                 <div className="mb-12">
                     <h3 className="font-black text-lg border-l-4 border-emerald-500 pl-4 mb-8">Bagan Pertandingan</h3>
                    ...
                    <table className="w-full text-[10px] font-black uppercase">
                       <thead className="bg-gray-50 text-gray-400">
                           <tr>
                               ...
                               <th className="p-4 text-center">Partai</th>
                               <th className="p-4 text-center">Jadwal</th>
                               <th className="p-4 text-right">Skor</th>
                           </tr>
                       </thead>
                       <tbody className="divide-y divide-gray-100">
                           {generateMasterPlan().map(m=>(
                               <tr key={m.id}>
                                  <td className="p-4 text-gray-300">#{m.id}</td>
                                  <td className="p-4">{m.label}</td>
                                  <td className="p-4 text-center">
                                      <div className="flex justify-center gap-4">
                                          <span>{m.teamA}</span><span className="text-gray-200">VS</span><span>{m.teamB}</span>
                                      </div>
                                  </td>
                                  <td className="p-4 text-center text-gray-400">{m.date} | {m.time}</td>
                                  <td className="p-4 text-right">
                                      <span className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full">{formatMatchScore(m)}</span>
                                  </td>
                               </tr>
                           ))}
                       </tbody>
