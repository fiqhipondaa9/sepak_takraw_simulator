...
export default function App() {
  ...
  const [stage, setStage] = useState(0); // 0: Setup, 1: Phase 1, 2: Phase 2, 3: Finals
  const [matchHistory, setMatchHistory] = useState([]); // Arsip jadwal yang sudah berlalu
  ...
  // Kategori & Disiplin (Dropdown Unified)
  ...
  // Kontrol Fase 2
  const [phase2Format, setPhase2Format] = useState('group'); // 'group' or 'knockout'
  const [phase2ByeSystem, setPhase2ByeSystem] = useState('seeding'); // 'standard' or 'seeding'

  // Turunan Kategori
  let eventCategory = 'single';
  let eventDiscipline = 'Regu';
  if (selectedEventFormat.includes('Team')) eventCategory = 'team';
  if (selectedEventFormat.includes('Double')) eventDiscipline = 'Double';
  else if (selectedEventFormat.includes('Regu')) eventDiscipline = 'Regu';
  else if (selectedEventFormat.includes('Quadrant')) eventDiscipline = 'Quadrant';
  else if (selectedEventFormat.includes('Mixed')) eventDiscipline = 'Mix';

  // --- CEK STATUS ---
  const isActivePhaseFinished = schedule.length > 0 && schedule.every(match => match.winner !== null);

  // --- FUNGSI MODE LAYAR PENUH ---
  ...
  const handleSaveFile = () => {
    const dataToSave = {
      stage, matchHistory, teams, schedule, tournamentType, selectedEventFormat, mixDisciplines, numGroups, 
  ...
  const handleOpenFile = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        setTournamentType(data.tournamentType || 'group'); setSelectedEventFormat(data.selectedEventFormat || 'Regu Event'); 
  ...
  // --- LOGIKA KLASEMEN (HEAD-TO-HEAD) ---
  const getStandings = (specificSchedule = schedule, specificAssignments = groupAssignments) => {
    let standings = {};
    teams.forEach(t => { standings[t] = { team: t, group: tournamentType === 'group' ? (specificAssignments[t] ? `Grup ${specificAssignments[t]}` : 'Pool Utama') : 'Pool Utama', play: 0, win: 0, lose: 0, partyWin: 0, partyLose: 0, setWin: 0, setLose: 0, pointWin: 0, pointLose: 0, totalPoints: 0 }; });

    specificSchedule.forEach(match => {
      if (!match.winner) return;
      const tA = match.teamA; const tB = match.teamB;
      if(standings[tA]) standings[tA].play += 1; if(standings[tB]) standings[tB].play += 1;
      
      if (match.winner === tA) { if(standings[tA]) standings[tA].win += 1; if(standings[tB]) standings[tB].lose += 1; } 
      else if (match.winner === tB) { if(standings[tB]) standings[tB].win += 1; if(standings[tA]) standings[tA].lose += 1; }
      else if (match.winner === 'SERI') { if(standings[tA]) standings[tA].lose += 1; if(standings[tB]) standings[tB].lose += 1; }

      let mPWA = 0; let mPWB = 0;
      match.parties.forEach(party => {
        if (party.winner === tA) { if(standings[tA]) standings[tA].partyWin += 1; if(standings[tB]) standings[tB].partyLose += 1; mPWA += 1; } 
        else if (party.winner === tB) { if(standings[tB]) standings[tB].partyWin += 1; if(standings[tA]) standings[tA].partyLose += 1; mPWB += 1; }
        else if (party.winner === 'SERI') { if(standings[tA]) standings[tA].partyLose += 1; if(standings[tB]) standings[tB].partyLose += 1; }

        party.sets.forEach(set => {
          const a = parseInt(set.scoreA); const b = parseInt(set.scoreB);
          if (!isNaN(a) && !isNaN(b)) {
            if (a > b) { if(standings[tA]) standings[tA].setWin += 1; if(standings[tB]) standings[tB].setLose += 1; } else if (b > a) { if(standings[tB]) standings[tB].setWin += 1; if(standings[tA]) standings[tA].setLose += 1; }
            if(standings[tA]) { standings[tA].pointWin += a; standings[tA].pointLose += b; }
            if(standings[tB]) { standings[tB].pointWin += b; standings[tB].pointLose += a; }
          }
        });
      });

      if (eventCategory === 'team') {
        if (match.winner === tA) { if(standings[tA]) standings[tA].totalPoints += (mPWB === 1) ? 2 : 3; if(standings[tB]) standings[tB].totalPoints += (mPWB === 1) ? 1 : 0; } 
        else if (match.winner === tB) { if(standings[tB]) standings[tB].totalPoints += (mPWA === 1) ? 2 : 3; if(standings[tA]) standings[tA].totalPoints += (mPWA === 1) ? 1 : 0; }
      } else {
        if (match.winner === tA) { if(standings[tA]) standings[tA].totalPoints += 2; } else if (match.winner === tB) { if(standings[tB]) standings[tB].totalPoints += 2; }
      }
    });

    const sortedTeams = Object.values(standings).sort((a, b) => {
      if (b.totalPoints !== a.totalPoints) return b.totalPoints - a.totalPoints;
      const h2hMatch = specificSchedule.find(m => ((m.teamA === a.team && m.teamB === b.team) || (m.teamA === b.team && m.teamB === a.team)) && m.winner);
      if (h2hMatch && h2hMatch.winner !== 'SERI') { if (h2hMatch.winner === a.team) return -1; if (h2hMatch.winner === b.team) return 1; }
      if (b.win !== a.win) return b.win - a.win;
      if (eventCategory === 'team') {
        const aP = a.partyWin - a.partyLose; const bP = b.partyWin - b.partyLose; if (bP !== aP) return bP - aP;
      }
  ...
  // --- PEMBUAT JADWAL FASE 1 ---
  const generateSchedule = () => {
    if (tournamentType === 'group' && !teams.every(team => groupAssignments[team])) return alert("Ada tim yang belum masuk ke dalam grup!");
    ...
    const createRR = (gTeams, label) => {
      ...
      const n = cur.length; for(let r=0; r<n-1; r++){ for(let i=0; i<n/2; i++){ if(cur[i] && cur[n-1-i]) sch.push({ teamA: cur[i], teamB: cur[n-1-i], groupLabel: label }); } cur.splice(1, 0, cur.pop()); } return sch;
    };
    ...
    const nP = eventCategory === 'team' ? 3 : 1;
    ...
    const addMins = eventCategory === 'team' ? 120 : 45;

    allMatches.forEach(m => {
      let pts = []; for(let p=0; p<nP; p++) pts.push({ id: `p${p}`, label: eventCategory === 'team' ? pL[p] : `Match`, sets: [{scoreA:'',scoreB:''},{scoreA:'',scoreB:''},{scoreA:'',scoreB:''}], winner: null });
      let eIdx = 0; for(let i=1; i<cTimes.length; i++) if(cTimes[i] < cTimes[eIdx]) eIdx = i;
      fSch.push({ id: counter++, teamA: m.teamA, teamB: m.teamB, groupLabel: m.groupLabel, parties: pts, winner: null, winsA: 0, winsB: 0, time: cTimes[eIdx].toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}), court: aC[eIdx] });
  ...
  const handleExecutePhase2 = () => {
    ...
    // Simpan Fase 1 ke histori
    setMatchHistory([...matchHistory, ...schedule]);

    if (phase2Format === 'group' && Number(numGroups) === 3 && q.length >= 6) {
       // GRUP D & E (Interleaved)
       ...
       setTeams([tA1, tB2, tC1, tB1, tA2, tC2]); setGroupAssignments(newAssigns); setTournamentType('group'); setNumGroups(2);

       // Interleaved Manually for D & E
       let fSch = []; let counter = schedule.length + 1;
       const aC = courts.length > 0 ? courts : ['Lap. Utama'];
       let cTimes = aC.map(() => { let t = new Date(); t.setHours(8, 0, 0, 0); return t; });
       const addMins = eventCategory === 'team' ? 120 : 45;
       const nP = eventCategory === 'team' ? 3 : 1;
       ...
       if (phase2ByeSystem === 'seeding') {
          ...
          ord[0] = q[0].team; ord[4] = q[1].team; ord[2] = q[3].team; ord[3] = q[4].team; ord[6] = q[2].team; ord[7] = q[5].team;
       } else {
          const gA = std['Grup A']||[]; const gB = std['Grup B']||[]; const gC = std['Grup C']||[];
          ord[0] = gA[0]?.team||'BYE'; ord[4] = gB[0]?.team||'BYE'; ord[2] = gC[0]?.team||'?'; ord[3] = gB[1]?.team||'?'; ord[6] = gC[1]?.team||'?'; ord[7] = gA[1]?.team||'?';
       }

       let rds = []; const nR = 3; // PO, SF, Final
       const nP = eventCategory === 'team' ? 3 : 1; const pL = eventDiscipline === 'Mix' ? mixDisciplines : [`${eventDiscipline} 1`, `${eventDiscipline} 2`, `${eventDiscipline} 3`];
       for(let r=0; r<nR; r++) {
          let c = size / Math.pow(2, r+1); let rMatches = [];
          for(let m=0; m<c; m++) {
             let tA = r===0 ? ord[m*2] : '?'; let tB = r===0 ? ord[m*2+1] : '?';
             let pts = []; for(let p=0; p<nP; p++) pts.push({ id:`k_p${p}`, label: eventCategory==='team'?pL[p]:`Match`, sets: [{scoreA:'',scoreB:''},{scoreA:'',scoreB:''},{scoreA:'',scoreB:''}], winner:null });
             let lbl = r===nR-1 ? 'FINAL' : r===nR-2 ? 'SEMI FINAL' : 'PLAY-OFF';
             rMatches.push({ id:`k_${r}_${m}`, roundIndex: r, matchIndex: m, title: lbl, teamA: tA, teamB: tB, parties: pts, winner: null, winsA: 0, winsB: 0, nextMatchRef: r+1<nR ? {r:r+1, m:Math.floor(m/2), slot:m%2===0?'teamA':'teamB'} : null });
          }
          rds.push(rMatches);
       }
       setSchedule([]); setKnockoutData(rds); setStage(3); window.scrollTo(0,0);
    }
  };

  // --- CROSS-SEMI DARI GRUP FASE 2 ---
  const handleGenerateSemiFinals = () => {
    const std = getStandings(); // get current Phase 2 standings
    ...
    setMatchHistory([...matchHistory, ...schedule]); // archive Phase 2

    let rds = []; const nR = 2; // SF, Final
    const ord = [tD1, tE2, tE1, tD2];
    const nP = eventCategory === 'team' ? 3 : 1; const pL = eventDiscipline === 'Mix' ? mixDisciplines : [`${eventDiscipline} 1`, `${eventDiscipline} 2`, `${eventDiscipline} 3`];
    for(let r=0; r<nR; r++) {
       let c = 4 / Math.pow(2, r+1); let rMatches = [];
       for(let m=0; m<c; m++) {
          let tA = r===0 ? ord[m*2] : '?'; let tB = r===0 ? ord[m*2+1] : '?';
          let pts = []; for(let p=0; p<nP; p++) pts.push({ id:`s_p${p}`, label: eventCategory==='team'?pL[p]:`Match`, sets: [{scoreA:'',scoreB:''},{scoreA:'',scoreB:''},{scoreA:'',scoreB:''}], winner:null });
          rMatches.push({ id:`s_${r}_${m}`, roundIndex: r, matchIndex: m, title: r===0?'SEMI FINAL':'FINAL', teamA: tA, teamB: tB, parties: pts, winner: null, winsA: 0, winsB: 0, nextMatchRef: r+1<nR ? {r:r+1, m:Math.floor(m/2), slot:m%2===0?'teamA':'teamB'} : null });
       }
       rds.push(rMatches);
    }
    setSchedule([]); setKnockoutData(rds); setStage(3); window.scrollTo(0,0);
  ...
  const renderMatchCard = (match, isKnockout = false, index = null) => {
    const isLive = (!match.teamA.includes('?') && !match.teamB.includes('?') && match.parties.some(p => p.sets.some(s => s.scoreA !== '' || s.scoreB !== '')) && !match.winner);
    return (
          ...
          {!isProjectorMode && stage > 0 && (
            <div className="no-print bg-white rounded-3xl shadow-sm p-4 flex flex-col md:flex-row justify-between items-center gap-4 border border-gray-100">
              <div className="flex items-center gap-3">
                 ...
                 <div><h3 className="font-black text-gray-800">Tournament Dashboard</h3><p className="text-xs text-gray-500 font-medium">Input skor atau lihat Jadwal Induk (Master Plan).</p></div>
              </div>
              <div className="flex flex-wrap gap-2 items-center">
                <button onClick={() => setShowMasterModal(true)} className={`${theme.soft} ${theme.textPrimary} hover:bg-gray-100 px-4 py-2.5 rounded-2xl flex items-center gap-2 text-sm font-black transition-colors`}><IconList /> JADWAL INDUK (LAPORAN EKSKULSIF)</button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 no-print">
              ...
              <div className="lg:col-span-5 flex flex-col gap-6">
                <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
                  ...
                  <div className="mb-6">
                    <select value={selectedEventFormat} onChange={(e) => setSelectedEventFormat(e.target.value)} className="w-full bg-gray-50 border border-gray-200 text-gray-800 font-black text-sm rounded-2xl px-5 py-4 outline-none focus:border-blue-400 focus:bg-white shadow-sm cursor-pointer transition-colors">
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 flex-1 flex flex-col">
                  ...
                  <div className="mb-6">
                    <form className="flex gap-2 mb-3"><input type="text" value={courtInputValue} onChange={(e) => setCourtInputValue(e.target.value)} placeholder="Tambah lapangan..." className="flex-1 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:bg-white text-xs font-bold" /><button type="button" onClick={handleAddCourt} className="bg-gray-800 hover:bg-black text-white px-4 py-3 rounded-xl font-bold transition-colors"><IconPlus /></button></form>
          ...
          {(stage === 1 || stage === 2) && schedule.length > 0 && (
            <div className={`space-y-6 animate-in fade-in duration-500 print:space-y-4 ${isProjectorMode ? 'space-y-10' : ''}`}>
                        ...
                        <table className="w-full text-left whitespace-nowrap">
                          <thead>
                            <tr className={`text-gray-400 bg-white border-b border-gray-50 ${isProjectorMode ? 'text-sm uppercase tracking-widest' : 'text-[10px] uppercase tracking-widest'}`}>
                              ...
                              {eventCategory === 'team' && <th className="px-2 py-4 font-black text-center bg-gray-50">Prt</th>}<th className="px-2 py-4 font-black text-center">Set</th><th className="px-2 py-4 font-black text-center">Pts</th><th className={`px-4 py-4 font-black text-center ${theme.soft} ${theme.textPrimary}`}>SCORE</th>
                            </tr>
                          </thead>
              ...
              <div className={`bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden print-border ${isProjectorMode ? 'shadow-2xl border-none' : ''}`}>
                <div className={`flex justify-between items-center border-b border-gray-100 print:bg-gray-200 print:text-black ${isProjectorMode ? 'p-8 bg-gray-50' : 'p-6 bg-white'}`}>
                  <div className="flex items-center gap-4"><div className={`p-3 rounded-2xl ${theme.soft} ${theme.textPrimary}`}><IconCalendar /></div><h2 className={`font-black text-gray-800 tracking-tight print:text-black ${isProjectorMode ? 'text-4xl' : 'text-xl'}`}>Jadwal Pertandingan</h2></div>
                  {!isProjectorMode && ( <div className="no-print flex items-center gap-2"><button onClick={handleClearScores} className="text-xs bg-amber-50 hover:bg-amber-100 text-amber-700 px-4 py-2.5 rounded-xl font-bold uppercase tracking-widest border border-amber-200">Bersihkan Skor</button><button onClick={handleReset} className="text-xs bg-red-50 hover:bg-red-100 text-red-600 px-4 py-2.5 rounded-xl font-bold uppercase tracking-widest border border-red-200">Reset Total</button></div> )}
                </div>
                       ...
                       <>
                         ...
                         {Number(numGroups) === 3 ? (
                           <div className="bg-gray-800/80 p-6 md:p-8 rounded-2xl border border-gray-700 text-left backdrop-blur-sm">
                             <div className="flex flex-col md:flex-row gap-8">
                                <div className="flex-1 space-y-4">
                                   ...
                                   <select value={phase2Format} onChange={(e) => setPhase2Format(e.target.value)} className="w-full bg-gray-900 border border-gray-600 text-white font-bold text-sm rounded-xl px-4 py-4 outline-none focus:border-blue-400 cursor-pointer">
                                ...
                                {phase2Format === 'knockout' && (
                                   <div className="flex-1 space-y-4 animate-in fade-in duration-300">
                                      ...
                                      <select value={phase2ByeSystem} onChange={(e) => setPhase2ByeSystem(e.target.value)} className="w-full bg-gray-900 border border-gray-600 text-white font-bold text-sm rounded-xl px-4 py-4 outline-none focus:border-emerald-400 cursor-pointer">
                                         <option value="seeding">Tim Terbaik (Global Seeding)</option>
                                         <option value="standard">Posisi Standar (Juara A & B)</option>
                                      </select>
                                      <p className="text-[10px] text-gray-400">Tim Terbaik = Mengambil 2 tim poin teringgi dari semua grup.</p>
                                   </div>
                                )}
                             </div>
                           ...
                           <div className="flex flex-wrap justify-center gap-4">
                              <button onClick={() => { setPhase2Format('knockout'); setPhase2ByeSystem('standard'); handleExecutePhase2(); }} className="bg-emerald-600 hover:bg-emerald-700 text-white font-black py-4 px-8 rounded-2xl shadow-md transition-all">Lanjut Sistem Gugur</button>
                           </div>
                         )}
                       </>
                      ...
                      <tbody>
                        {generateMasterPlan().map((m, idx) => (
                          <tr key={idx} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                            <td className="p-4 font-black text-gray-300">#{m.id !== 'TBD' && !String(m.id).includes('K') && !String(m.id).includes('F') ? m.id : '-'}</td>
