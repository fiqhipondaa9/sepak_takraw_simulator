...
export const RegistrationForm = ({
  theme, teams, setTeams, inputValue, setInputValue,
  teamLogos, handleTeamLogoUpload, groupAssignments, setGroupAssignments, 
  numGroups, setNumGroups, handleRemoveTeam, selectedEventFormat, setSelectedEventFormat,
  mixDisciplines, setMixDisciplines, courtInputValue, setCourtInputValue, 
  handleAddCourt, courts, handleRemoveCourt, tournamentType, setTournamentType,
  roundRobinType, setRoundRobinType, handleAutoAssign, onStartTournament,
  // State baru untuk huruf awal grup
  groupStartOffset, setGroupStartOffset
}) => {
  ...
  // Fungsi khusus agar tombol "Susun" menghormati huruf awalan grup yang dipilih
  const handleLocalAutoAssign = () => {
    let newAssignments = {};
    const groupNames = Array.from({length: Number(numGroups) || 2}, (_, i) => String.fromCharCode(65 + (groupStartOffset || 0) + i));
    teams.forEach((team, index) => {
      newAssignments[team] = groupNames[index % groupNames.length];
    });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 no-print">
      ...
      <div className="lg:col-span-7 bg-white rounded-3xl shadow-sm border border-gray-100 p-6 md:p-8 flex flex-col h-full">
        ...
        <form onSubmit={handleAddTeamSubmit} className="flex gap-3 mb-6">
          <input 
            type="text" maxLength={40} value={inputValue} 
              ...
              <div className="flex items-center gap-3">
                {tournamentType === 'Groups' && (
                  ...
                  >
                    ...
                    {/* Menggunakan groupStartOffset agar dropdown sesuai dengan pilihan awalan grup */}
                    {Array.from({length: Number(numGroups) || 2}, (_, i) => String.fromCharCode(65 + (groupStartOffset || 0) + i)).map(g => (
      ...
      <div className="lg:col-span-5 flex flex-col gap-6">
            ...
            <div className={`p-4 rounded-2xl border ${theme.border} ${theme.soft} flex flex-col gap-3`}>
              {[0, 1, 2].map((idx) => (
                <div key={idx} className="flex flex-col gap-1">
                  ...
                  <select value={mixDisciplines[idx]} onChange={(e) => { const newMix = [...mixDisciplines]; newMix[idx] = e.target.value; setMixDisciplines(newMix); }} className="w-full bg-white border border-gray-200 text-gray-800 font-bold text-xs rounded-xl px-3 py-2 uppercase">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 flex-1 flex flex-col">
          ...
          <div className="flex gap-2 mb-3">
            <input type="text" value={courtInputValue} onChange={(e) => setCourtInputValue(e.target.value)} placeholder="Tambah lapangan..." className="flex-1 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:bg-white text-xs font-bold uppercase" />
            <button type="button" onClick={handleAddCourt} className="bg-gray-800 hover:bg-black text-white px-4 py-3 rounded-xl font-bold transition-colors"><Icons.IconPlus /></button>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {courts.map((court, index) => ( 
              <div key={index} className="bg-gray-100 text-gray-700 px-3 py-2 rounded-xl text-[10px] font-black flex items-center gap-2 border border-gray-200">
                {court} {courts.length > 1 && <button type="button" onClick={() => handleRemoveCourt(court)} className="text-gray-400 hover:text-red-500"><Icons.IconTrash /></button>}
              </div> 
            ))}
          </div>
          ...
          {(tournamentType === 'Group' || tournamentType === 'Groups') && (
            <select value={roundRobinType} onChange={(e) => setRoundRobinType(e.target.value)} className="w-full bg-gray-50 border border-gray-200 text-gray-700 font-bold text-xs rounded-xl px-4 py-3 outline-none cursor-pointer mb-4 uppercase">
          ...
          {tournamentType === 'Groups' && (
            <div className="flex flex-col gap-3 mb-4">
              {/* FITUR BARU: Pemilihan Huruf Awalan Grup */}
              <div className={`p-4 rounded-xl border ${theme.border} ${theme.soft} flex justify-between items-center`}>
                <span className="font-black text-xs">MULAI DARI GRUP</span>
                <select 
                  value={groupStartOffset || 0} 
                  onChange={(e) => setGroupStartOffset(Number(e.target.value))} 
                  className="bg-white border border-gray-200 text-gray-700 font-black text-xs rounded-lg px-3 h-10 outline-none shadow-sm cursor-pointer"
                >
                  {Array.from({length: 15}, (_, i) => (
                    <option key={i} value={i}>Grup {String.fromCharCode(65 + i)}</option>
                  ))}
                </select>
              </div>
              ...
              <div className={`p-4 rounded-xl border ${theme.border} ${theme.soft} flex justify-between items-center`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {/* Gunakan handleLocalAutoAssign di sini */}
                  <button onClick={handleLocalAutoAssign} className="bg-white border border-gray-200 text-gray-700 font-black text-[10px] px-3 h-10 rounded-lg hover:bg-gray-50 uppercase shadow-sm">Susun</button>
                </div>
              </div>
            </div>
          )}
          
          <button onClick={onStartTournament} disabled={teams.length < 2} className={`w-full mt-auto ${theme.accent} ${theme.accentHover} ${theme.accentText} disabled:opacity-50 font-black py-4 rounded-xl shadow-md transition-all text-lg flex justify-center items-center gap-2`}>
